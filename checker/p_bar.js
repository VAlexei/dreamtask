...
//методы для добавления и удаления задач в tasks
    ...
    // убедиться, что было что-либо введено
        ...
        this.tasks.push({ name: this.newtask, inwrk: false, whoinwork: '', check: false, men: false, red: false });
        this.newtask = '';
...
//Общий метод для удаления подзадач (везде)
    ...
    tasknaruto(x){
     this.tasks.splice(x + 1, 0, { name: this.tasks[x].name, inwrk: false, whoinwork: '', check: false, men: false, red: false });
